...
use Cungfoo\Command\Command as BaseCommand,
    Cungfoo\Lib\Resalys\Client\PrixClient;

class PrixCommand extends BaseCommand
{
    protected function configure()
    {
        $this
            ->setName('resalys:load:prix')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        try
        {
            $client = new PrixClient($this->getApplication()->getRootDir());
        ...
        {
            $output->writeln(sprintf('<info>resalys:load:prix</info> <error>error %s:</error>.', $exception->getMessage()));
            return false;
        }

        $output->writeln(sprintf('<info>resalys:load:prix</info> <info>data is loaded :D</info>.'));
        return true;
    }
}