...
abstract class BaseJobLogQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'cungfoo', $modelName = 'Cungfoo\\Model\\JobLog', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Cungfoo\Model\JobQuery A secondary query class using the current class as primary query
     */
    public function useJobQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinJob($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Job', '\Cungfoo\Model\JobQuery');
    }
