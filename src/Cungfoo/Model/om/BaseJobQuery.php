...
abstract class BaseJobQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'cungfoo', $modelName = 'Cungfoo\\Model\\Job', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Cungfoo\Model\JobLogQuery A secondary query class using the current class as primary query
     */
    public function useJobLogQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinJobLog($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'JobLog', '\Cungfoo\Model\JobLogQuery');
    }
