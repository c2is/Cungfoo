(function($) {
    ...
    $.c2is.portfolio = function(el, options) {
        var base = this;

        base.$el = $(el);
        base.el  = el;

        base.$el.data("c2is.portfolio", base);

        base.init = function() {
            base.options = $.extend({},$.c2is.portfolio.defaultOptions, options, base.$el.data());
        ...
        base.editFormMedia = function(e) {
            ...
            if (linkEdit.hasClass('on')) {
                $.get(linkEdit.attr('href'), function (html) {
                    mediaItem.after(html);
                });
            }
        ...
        base.validate = function(e) {
            ...
            if (linkPopin.children('img').length == 0) {
                linkPopin.attr('href', linkPopin.attr('href') + '/' + sMediaIds);
            }
            else {
    ...
    $.fn.c2is_portfolio = function(options) {
        return this.each(function() {
            (new $.c2is.portfolio(this, options));
        });
    };

    $.fn.getc2is_portfolio = function() {
        this.data("c2is.portfolio");
    };
